...
import { HORARIOS, SITE_CONTACT } from "@/lib/data";
...
export function Horarios({
  ...
  return (
    <section
      className={`relative ${compact ? "py-20 md:py-28" : "py-28 md:py-40"}`}
      aria-label="Horários de treino"
    >
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
        ...
        <div
          ...
          className={`flex flex-wrap gap-3 ${compact ? "mt-8" : "mt-12"}`}
        >
          {TABS.map((t) => (
            <button
              key={t}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`divide-y divide-line border-y border-line ${compact ? "mt-8" : "mt-12"}`}
          >
        ...
        <p className="mt-8 text-xs text-fg-muted">
          * Grade sujeita a ajustes. Confirme horários na recepção ou pelo Instagram {SITE_CONTACT.instagramHandle}.
        </p>
      </div>
    </section>
  );
